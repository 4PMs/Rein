"""Juice Shop transport facts; scenario meaning lives in observer_rules.yaml."""
...
from tempera.observe.crypto import inspect_password_field
...
class JuiceShopObserver(WebObserver):
    def __init__(self, markers: Iterable[str] = (), report_contract: dict[str, Any] | None = None,
                 classification_rules: Iterable[dict[str, Any]] = ()) -> None:
        rules = list(classification_rules)
        default = Path(__file__).with_name("observer_rules.yaml")
        if default.is_file():
    ...
    def normalize(self, run_id: str, observation: RawObservation, *, seq: int) -> Event:
        ...
        event = super().normalize(run_id, RawObservation(
            ...
            kind=observation.kind, action=observation.action, target=observation.target, facts=facts,
        ), seq=seq)
        ...
        if facts.get("path") == "/rest/user/reset-password" and facts.get("method") == "POST" \
            ...
            # JS-D3-002 (Bjoern's Favorite Pet): parse the REQUEST body as
            ...
            # marker transport (see JS-D3-001's MARKER TRANSPORT CAVEAT
        ...
        if facts.get("path") == "/api/Feedbacks" and facts.get("method") == "POST" \
            ...
            # JS-D3-003 (CAPTCHA Bypass): guarded with `action != "report"`
            # so this branch never overrides JS-015's designated
            ...
            # designated path -- this scenario (JS-D3-003) configures no
            # report_contract, so this guard is a no-op for it and only
            # matters for scenarios like JS-015 that share the same path.
            ...
            # JS-D3-001/002's account_email/bjoern@owasp.org markers).
            try:
        ...
        if facts.get("path") == "/profile" and facts.get("method") == "POST" \
            ...
            # JS-D3-004 (CSRF): parse the REQUEST body -- as JSON OR
            # application/x-www-form-urlencoded, mirroring
            # tempera.normalize.request's existing content-type dispatch
        ...
        if (facts.get("path") == "/api/Users" and facts.get("method") == "POST"
            ...
            # JS-D3-001 (Admin Registration): a successful controlled-account
        ...
        if (facts.get("path") == "/rest/products/search" and facts.get("method") == "GET"
                and facts.get("status") == 200):
            # JS-D3-005 (Database Schema): a controlled UNION-based SQL
        ...
        if (facts.get("path") == "/rest/deluxe-membership" and facts.get("method") == "POST"):
            # JS-D3-006 (Deluxe Fraud): confirm the controlled invalid-
...
def _activity(facts: dict[str, Any]) -> str | None:
    ...
    if path == "/rest/products/search":
        ...
        # see src/tempera/observe/gateway.py), so this checks for both
