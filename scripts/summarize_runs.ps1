<#
.SYNOPSIS
    Aggregate already-executed Tempera Kali Container Agent Mode runs by
    experiment id, split by restraint/capability condition, and print/save a
    summary.

.DESCRIPTION
    Read-only aggregation tool. It never modifies, deletes, or recomputes
    anything in runs/<run_id>/result.json -- status, termination,
    goal.success, roe.compliant, roe.violations, roe.unclassified, usage.*,
    metrics.*, timing.*, and declarations.* are read verbatim as the
    runner/evaluator already wrote them. This script performs no additional
    judgement about goal or RoE outcomes: it only counts, formats, and
    averages fields that already exist in result.json.

    Run discovery: every result.json is found recursively under -RunsRoot,
    then kept only if -Experiment is a substring of either the file's full
    path or the run_id field inside result.json. This matches the
    run-$Scenario-$Experiment-<condition>-r<N> naming convention produced by
    run_container_experiment.ps1, but does not require it -- any run whose
    id or path contains the experiment string is included.

    Condition classification is purely textual, from the run_id:
      - if run_id contains "capability" (case-insensitive)  -> capability
      - elif run_id contains "restraint" (case-insensitive) -> restraint
      - else                                                -> unknown
    This never reads or reinterprets any agent self-report; the condition
    label is only used to group already-computed result.json metrics.

    Rate fields: GoalSuccessRate/RoeCompliantRate are stored as 0-1 fractions
    in summary.json under goal_success_rate/roe_compliant_rate (source of
    truth, unchanged from the previous version of this script), plus
    goal_success_percent/roe_compliant_percent (the same value * 100,
    rounded) for convenience. Console and Markdown output always render the
    percent form ("100%") since that is what a human skimming a report
    needs; nothing about the underlying fraction changes.

.PARAMETER Experiment
    Required. Substring to match against each result.json's path or run_id.

.PARAMETER RunsRoot
    Root directory to search recursively for result.json files. Default: runs

.PARAMETER ShowRuns
    Switch. Also print a per-run table: RunId, Condition, Status, Valid,
    Termination, GoalSuccess, RoeCompliant, Violations, Unclassified, Steps,
    ModelCalls, TotalTokens, DurationSec.

.PARAMETER ShowLatestEvents
    Switch. Also print the most recently modified matching run's
    events.jsonl, one row per event: Seq, Method, Path, Status, Markers,
    Activity, Outcome. Read-only.

.PARAMETER ShowLatestDeclarations
    Switch. Also print the most recently modified matching run's
    declarations block from result.json (confirmed, phantom, covert,
    mismatch, declaration_accuracy, covert_rate, status_mismatch_count).
    Read-only.

.PARAMETER Table
    Switch. Render the per-condition summary as a Format-Table instead of
    the default Format-List. Format-List is the default because the full
    metric set (13 columns) does not fit a typical console width without
    wrapping; -Table is available for anyone who prefers the compact form
    and has a wide enough terminal.

.PARAMETER OutDir
    Directory to write the summary JSON and Markdown files into.
    Default: runs/_summaries

.EXAMPLE
    .\scripts\summarize_runs.ps1 -Experiment kali-js006-20260912-173210 -ShowRuns -ShowLatestEvents -ShowLatestDeclarations
#>
[CmdletBinding()]
param(
...
function Get-Average {
    param([double[]]$Values)
    # $Values is [double[]]-typed, so PowerShell always coerces the bound
    # argument into an array even for a single scalar input -- but the
    # *filtered* result of Where-Object on a single-element array can
    # still collapse back to a bare scalar. When that scalar happens to be
    # 0, "-not $clean" evaluates to $true (0 is falsy in PowerShell boolean
    # context), which incorrectly short-circuited this function to return
    # $null instead of the real average of 0 -- observed live: a
    # single-run condition where Violations/Unclassified were legitimately
    # 0 showed "n/a" instead of "0" in both the console and summary.json.
    # Wrapping in @(...) forces array semantics so Count and emptiness
    # checks are unambiguous regardless of how many non-null values there
    # are or what their value is.
...
function Format-Percent {
    # Renders a 0-1 fraction as a human string like "100%" or "n/a" for
    # display only; the underlying fraction (unrounded beyond Get-Rate's
    # 4 decimal places) is what actually gets persisted to summary.json.
    param($Fraction)
...
function Format-Value {
    # Renders $null as "n/a" instead of an empty string so gaps in
    # optional result.json fields (duration_sec, usage, roe.summary, etc.)
...
# --- 2. Parse and filter by Experiment (path OR run_id substring match) -----
...
foreach ($file in $allResultFiles) {
    ...
    try {
        ...
        # Only warn about parse failures for files whose path already
        # matched -- an unrelated broken result.json elsewhere under
        # RunsRoot is not this experiment's problem.
        if ($pathMatches) {
...
# --- 5. Persist summary JSON + Markdown (read-only w.r.t. runs/*) -----------
