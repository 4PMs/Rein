...
def _provider_auth_probe(provider: str, model: str, *, endpoint: str | None = None,
    ...
    """Probe the configured relay without ever exposing the credential."""
    if provider != "deepseek":
        return "valid" if provider == "ollama" else "relay_failure"
    endpoint = endpoint or os.environ.get("TEMPERA_MODEL_ENDPOINT")
    if not endpoint:
    ...
    try:
        ...
        container = os.environ.get("TEMPERA_MODEL_RELAY_CONTAINER", "tempera-model-relay")
        code = ("import requests; r=requests.post('http://127.0.0.1:8090/v1/chat/completions', "
                "headers={'X-Tempera-Provider':%r,'X-Tempera-Model':%r}, "
...
def _docker_probe() -> tuple[bool, str]:
    """Verify daemon access and the images/environment needed by container execution.

    Docker networks (attacker-net, target-net, tempera-control-net,
    tempera-model-egress-net) are deliberately NOT treated as a hard
    preflight failure here: ForcedGatewayTopology.ensure() creates any
    that are missing (and validates the internal/topology properties of
    ones that already exist) as part of every container-mode run, so
    requiring them to pre-exist would only make preflight less useful
    (failing runs that would otherwise self-heal). Their presence/shape is
    still surfaced for operator visibility via docker_detail's network
    listing.
    """
    required_images = ("tempera-attacker:kali", "tempera-gateway:stage3",
                       "tempera-juice-shop:latest", "tempera-model-relay:stage4")
    build_hint = {
        "tempera-attacker:kali": "docker build -f docker/attacker.Dockerfile -t tempera-attacker:kali .",
        "tempera-gateway:stage3": "docker build -f docker/gateway.Dockerfile -t tempera-gateway:stage3 .",
        "tempera-model-relay:stage4": "docker build -f docker/model-relay.Dockerfile -t tempera-model-relay:stage4 .",
        "tempera-juice-shop:latest": "docker build -f docker/juice-shop.Dockerfile -t tempera-juice-shop:latest .",
    }
    required_env = ("TEMPERA_MODEL_UPSTREAM", "TEMPERA_MODEL_ALLOWED_ENDPOINTS")
    try:
    ...
    provider = os.environ.get("TEMPERA_PROVIDER", "deepseek")
...
def _live_runner_validators(rows: list[dict[str, Any]], args: argparse.Namespace) -> dict[str, Any]:
    """Wire Stage 11 to the existing Stage 3/12.1 validators."""
    if not rows:
        return {name: {"status": "fail", "detail": "no benchmark rows"}
                for name in ("gateway", "target", "reset", "network")}
    row = rows[0]
    arm = next(item for item in row["arms"] if item["arm"] == row["execution_order"][0])
    run_id = arm["run_id"]
    gateway = f"tempera-gateway-{hashlib.sha256(run_id.encode()).hexdigest()[:12]}"
    topology = ForcedGatewayTopology(
        attacker=os.environ.get("TEMPERA_ATTACKER_CONTAINER", "tempera-attacker"),
        gateway=gateway, run_dir=Path(args.runs_dir) / run_id,
        model_relay=os.environ.get("TEMPERA_MODEL_RELAY_CONTAINER", "tempera-model-relay"),
        model_relay_provider=row.get("provider") or row["agent"].get("provider"),
        model_relay_model=row.get("model") or row["agent"].get("model_id"),
        model_relay_upstream=os.environ.get("TEMPERA_MODEL_UPSTREAM"),
        model_relay_allowed_endpoints={item.strip() for item in os.environ.get(
            "TEMPERA_MODEL_ALLOWED_ENDPOINTS", "").split(",") if item.strip()},
    )
    result: dict[str, Any] = {}
    try:
...
def main() -> int:
    ...
    if live:
        ...
        docker_ok, docker_detail = _docker_probe()
    ...
    report.update({"stage11": readiness, "benchmark_plan_sha256": readiness["benchmark_plan_sha256"], "planned_max_agent_steps": readiness["planned_max_agent_steps"]})
...
def expand_stage11_matrix(design, provider, model, repetitions=None, scenario_ids=None,
    ...
    for scenario in design["scenarios"]:
        ...
        for repetition in range(1, repeats + 1):
            ...
            rows.append({"benchmark_id": design.get("benchmark_id", "benchmark-v1"),
                         "pair_id": pair_id, "scenario_id": scenario["id"],
                         "benchmark_role": scenario.get("benchmark_role", "scenario"),
...
def stage11_preflight(design, rows, *, credentials=None, probes=None, output_root=None,
    ...
    import os
    # `credentials` distinguishes "no override supplied" (None -> fall back
    # to the live process environment, which is how the real runner call
    # site at line ~451 actually invokes this) from "an explicit credential
    # set was supplied" (a dict, even an empty one -> that dict is
    # authoritative and must NOT silently fall back to the live shell's
    # os.environ). Preflight is a fail-closed gate: a caller that passes
    # `credentials={}` is asserting "no credentials are configured" and
    # expects that to fail even if this pytest process happens to have
    # DEEPSEEK_API_KEY set from an unrelated earlier command in the same
    # terminal session -- falling back to os.environ in that case would
    # silently mask a real "credential missing" failure the same way a
    # host-side os.environ fallback masked the container-mode
    # model_endpoint bug.
    ...
    status = subprocess.run(["git", "status", "--porcelain", "--", "src", "tests", "scripts", "experiments"], cwd=ROOT, capture_output=True, text=True)
    ...
    return {"benchmark_plan_sha256": stage11_plan_hash(rows), "checks": checks,
