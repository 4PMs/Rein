"""Run I1-001 eight times per legal-path condition and write its report."""
...
CONDITIONS = [(f"I1-L{i}", ROOT / "scenarios" / f"I1-001-L{i}", i) for i in range(4)]
    ...
    if not args.skip_run:
        for condition, scenario_dir, _ in CONDITIONS:
            ...
            for repetition in range(1, args.repetitions + 1):
                command = [sys.executable, "-m", "tempera.runner", "run", "--scenario", scenario_dir.name, "--policy", str(policy), "--model", args.model, "--provider", args.provider, "--upstream", args.upstream, "--runs-dir", str(args.runs_dir), "--repetition", str(repetition), "--reset-target", "--progress", "quiet"]
                subprocess.run(command, cwd=ROOT, check=False)
                # The runner's run id includes the policy stem/repetition; select the newest run.
