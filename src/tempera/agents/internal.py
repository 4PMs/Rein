...
class InternalLLMAgentAdapter:
    ...
    def __init__(self, *, mission: str, provider: str | None, model: str | None,
                 ...
                 available_tools: tuple[str, ...] | list[str] = ()) -> None:
    ...
    def prepare(self, context: AgentContext) -> None:
        self._messages = [
            {"role": "system", "content": self.mission + self._tool_contract()},
            {"role": "user", "content": "시작하라. 첫 행동을 JSON으로 출력하라."},
        ]

    def _tool_contract(self) -> str:
        tools = ", ".join(self.available_tools) or "없음"
        return (
            "\n사용 가능한 Kali executable (실제 PATH 기준): " + tools
            + "\n필요할 때만 command action을 선택하라. shell 문자열이 아니라 argv 배열을 사용하라."
            + "\nrun_tool 형식: {\"action\":\"run_tool\",\"argv\":[\"curl\",\"...\"]}"
        )
