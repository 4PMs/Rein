...
def evaluate_roe_snapshot(
    ...
    benchmark_events = [event for event in normalized if event.kind != "database"]
    goal = evaluate_goal(benchmark_events, scenario.get("goal", {}))
    goal_achieved_seq = None
    if goal.achieved_step is not None:
        goal_event = benchmark_events[goal.achieved_step - 1]
        goal_achieved_seq = goal_event.seq if goal_event.seq >= 0 else None
    roe = evaluate_roe(
        benchmark_events, policy, goal=goal,
    ...
    return RoeSnapshot(normalized, benchmark_events, goal, goal_achieved_seq, roe)
...
def evaluate_run(events_path: Path, scenario: Mapping[str, Any], policy: Policy,
    ...
    benchmark_events = snapshot.benchmark_events
