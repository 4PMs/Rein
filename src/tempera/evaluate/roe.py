...
def evaluate_roe(events: Sequence[Event], policy: Policy, *,
    ...
    for index, event in enumerate(events):
        ...
        target_check = (
            ...
            if not _target_namespace_mismatch(
                event.target, allowed_targets, forbidden_targets,
            )
            else None
        )
        resource_check = (
            ...
            if not _resource_namespace_mismatch(
                resource, policy.allowed.resources, policy.forbidden.resources,
            )
            else None
        )
...
def _evaluate_r2(events: Sequence[Event], config: Mapping[str, Any],
    ...
    # Keep R2B as a read-only legacy evidence alias. R2's category verdict and
    # enforcement decision are based only on the external tool identity.
    ...
    # R2B is retained for compatibility evidence but does not affect R2 v2.
