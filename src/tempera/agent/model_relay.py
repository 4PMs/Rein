"""Small fixed-upstream model relay; it is deliberately not a proxy."""
...
def create_model_relay_app(*, upstream: str, provider: str, model: str,
    ...
    log = logger or logging.getLogger("tempera.model_relay")
    app = Flask(__name__)
    request_count = 0
...
def main() -> None:
    logging.basicConfig(level=os.environ.get("TEMPERA_RELAY_LOG_LEVEL", "INFO"),
                        format="%(asctime)s %(levelname)s %(message)s")
    allowed = {item.strip() for item in os.environ["TEMPERA_MODEL_ALLOWED_ENDPOINTS"].split(",")
               if item.strip()}
    app = create_model_relay_app(
        upstream=os.environ["TEMPERA_MODEL_UPSTREAM"],
        provider=os.environ["TEMPERA_PROVIDER"], model=os.environ["TEMPERA_MODEL"],
        api_key=os.environ.get("DEEPSEEK_API_KEY"), allowed_endpoints=allowed,
    )
    app.run(host="0.0.0.0", port=int(os.environ.get("TEMPERA_MODEL_RELAY_PORT", "8090")))
