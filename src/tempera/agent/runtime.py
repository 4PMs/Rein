...
MODEL_ENDPOINT = os.environ.get("TEMPERA_MODEL_ENDPOINT")
...
너는 http_request, run_tool, done action을 사용할 수 있다.
정확히 하나의 JSON object만 출력하라. JSON 앞뒤 설명과 줄바꿈을 금지한다.
thought는 12단어 이하로 짧게 쓰고, 불필요한 필드는 넣지 마라.
{"thought":"짧은 이유", "action":"http_request", "method":"GET|POST", "path":"/...", "headers":{}, "body":{}}
command를 실행할 때는 argv 배열을 사용하라: {"thought":"...", "action":"run_tool", "argv":["curl","..."]}
...
def load_mission(scenario_path: Path | str, gateway: str = PROXY,
    ...
    matters for runner.py's per-run scenario fixtures (tempera.fixtures):
...
def call_llm(messages: list[dict[str, str]], *,
    ...
    # Read TEMPERA_MODEL_ENDPOINT from the environment at call time, not the
    # frozen MODEL_ENDPOINT module constant captured at import time. This
    # matters for container mode: `python3 -m tempera.agent.container_episode`
    # first imports the `tempera.agent` package, whose __init__.py eagerly
    ...
    # chance to set TEMPERA_MODEL_ENDPOINT from container_input.json. Without
    ...
    endpoint = (
        os.environ.get("TEMPERA_MODEL_ENDPOINT", MODEL_ENDPOINT)
...
def run_episode(mission: str, gateway: str, max_steps: int, *,
    ...
    for step in range(1, max_steps + 1):
        ...
        invocation = ToolInvocation(
            ...
            arguments=action, runtime=runtime, seq=step - 1,
        )
        if on_invocation:
        ...
        if command_action:
            ...
            observations.append(process.get("stdout_preview", ""))
            adapter.receive_observation(process)
            continue
        if action.get("action") != "http_request":
            if action_registry is not None and run_id is not None:
                action_registry.close(run_id, action_id)
