...
CONTROL_NETWORK = "tempera-control-net"
MODEL_EGRESS_NETWORK = "tempera-model-egress-net"
DEFAULT_GATEWAY_CONTAINER = "tempera-gateway-forced"
DEFAULT_TARGET_CONTAINER = "tempera-juice-forced"
ATTACKER_IMAGE = "tempera-attacker:kali"
GATEWAY_IMAGE = "tempera-gateway:stage3"
TARGET_IMAGE = "tempera-juice-shop:latest"
MODEL_RELAY_IMAGE = "tempera-model-relay:stage4"
DEFAULT_MODEL_RELAY_CONTAINER = "tempera-model-relay"
...
class ForcedGatewayTopology:
    ...
    def __init__(self, *, attacker: str = DEFAULT_ATTACKER_CONTAINER,
        ...
        # Instance-tunable so tests exercising the "container stays running"
        # path don't pay the real settle-window delay; production code
        # never overrides these (see _await_gateway_running's docstring for
        # why the settle window itself is needed).
    ...
    def _attacker_matches(self, name: str) -> bool:
        """Verify the attacker's runs-root bind mount, if a run_dir is configured.

        Only the runs *root* (the parent of the current arm's run_dir) is
        checked: unlike the gateway (which is recreated per run because its
        upstream/config differ per arm), the attacker container is long-lived
        across runs and only needs a stable mount of the shared runs
        directory at /app/runs so container_episode.py can always resolve
        /app/runs/<run_id> for whichever run is currently executing.
        """
        if not self.run_dir:
            return True
        ...
        identity_ok = self._canonical_path(labels.get("com.tempera.runs_root", "")) == expected_source
        return source_ok and identity_ok

    def _attacker_owned(self) -> bool:
        _, labels = self._attacker_inspect(self.topology.attacker)
        return labels.get("com.tempera.owner") == "benchmark-attacker"
    ...
    def _ensure_attacker(self) -> None:
        ...
        if exists and self.run_dir and not self._attacker_matches(name):
            if not self._attacker_owned():
                raise TopologyError("attacker_mount_mismatch: existing attacker is not benchmark-owned")
            self._docker("rm", "-f", name)
            exists = False
        if not exists:
            args = [
                "run", "-d", "--name", name, "--network", ATTACK_NETWORK,
                "--label", "com.tempera.owner=benchmark-attacker",
            ]
            if self.run_dir:
                args.extend([
                    "--label", f"com.tempera.runs_root={self._canonical_path(str(self.run_dir.parent))}",
    ...
    def _gateway_matches(self, run_id: str, upstream: str) -> bool:
        ...
        identity_ok = not labels.get("com.tempera.run_id") or labels.get("com.tempera.run_id") == run_id
        path_ok = not labels.get("com.tempera.artifact_path") or self._canonical_path(labels["com.tempera.artifact_path"]) == expected_source
    ...
    def _gateway_owned(self) -> bool:
        ...
        return labels.get("com.tempera.owner") == "benchmark-forced-gateway"
    ...
    def _await_gateway_running(self, name: str) -> bool:
        """Poll .State.Running for a short settle window rather than
        checking it once immediately after `docker run -d`/`docker start`.

        `docker run -d` returns as soon as the container process is
        forked, not once it has finished starting -- a container whose
        entrypoint crashes on startup (e.g. a Python process that raises
        during module-level config parsing, which is exactly what
        happened with a stale GATEWAY_IMAGE and a config.json schema
        mismatch) can still report .State.Running == "true" for a brief
        moment before Docker marks it Exited. A single immediate
        _running() check after docker run -d observed this exact false
        positive live during verification of this fix: `docker run -d`
        returned, `_running()` reported True, and ~1 second later the
        container had already exited -- with the container agent
        receiving a hostname nothing was listening behind. Polling with a
        short settle window catches the crash before returning control to
        the caller instead of a single point-in-time race.

        Attempts/interval are read from self._gateway_poll_attempts/
        self._gateway_poll_interval (constructor defaults 10/0.3s, i.e. up
        to ~3s) rather than hardcoded, so tests exercising the "stays
        running" path can override them to avoid the real delay while
        production always uses the real settle window.
        """
        import time
    ...
    def _ensure_gateway(self, run_id: str, *, observer_ref: str | None = None,
        ...
        if exists and self.run_dir and not self._gateway_matches(run_id, upstream):
            if not self._gateway_owned():
                raise TopologyError("gateway_mount_mismatch: existing gateway is not benchmark-owned")
            self._docker("rm", "-f", name)
            exists = False
        if not exists:
            args = [
                "run", "-d", "--name", name, "--network", ATTACK_NETWORK,
                "--label", "com.tempera.owner=benchmark-forced-gateway",
                "--label", f"com.tempera.run_id={run_id}",
            ]
            if self.run_dir:
                args.extend([
                    "--label", f"com.tempera.artifact_path={self.run_dir}",
            ...
            # Fail closed here, the same way _ensure_model_relay() already
            # does for the model relay container: without this check, a
            # gateway that crashes immediately after `docker run -d` (e.g.
            # a stale GATEWAY_IMAGE whose bundled tempera.core.run.RunConfig
            # rejects a config.json field the current Host writes -- exactly
            # what happened here) is silently left "not exists, not running"
            # while topology.ensure() reports success. The container agent
            # then gets a gateway hostname with nothing listening or even
            # DNS-registered behind it, and fails minutes later with an
            # opaque "curl: could not resolve host <gateway>" that gives no
            # hint the real problem was the gateway process crashing on
            # startup. _await_gateway_running() (not a single _running()
            # check -- see its docstring for why that raced) turns that
            # into an immediate, attributable TopologyError with the crash
            # log attached, before the container agent episode ever starts.
        ...
        if not self._running(name):
            # Belt-and-suspenders: a gateway that was running immediately
            # after startup but crashed while the network-connect calls
            # above were being applied (e.g. it was still initializing and
            # hit its own fatal error slightly later) must still fail
            # closed here rather than let ensure()/validate() report a
            # topology that looks structurally correct (network membership
            # is a static Docker-side fact even for a dead container) while
            # nothing is actually listening on the gateway hostname.
            raise TopologyError(
    ...
    def _model_relay_config(self) -> tuple[str, str, str, set[str]]:
        # No os.environ fallback here: the sole production call site
        # (src/tempera/runner.py's container branch) already reads
        # TEMPERA_MODEL_UPSTREAM/TEMPERA_PROVIDER/TEMPERA_MODEL/
        # TEMPERA_MODEL_ALLOWED_ENDPOINTS itself and passes them explicitly
        # as constructor kwargs. A redundant os.environ fallback here would
        # let an operator's ambient/leftover shell variables silently
        # satisfy this fail-closed completeness check even when the caller
        # supplied no config at all -- the same host-environment-leak
        # failure mode fixed for ContainerAgentRuntime.run()'s
        # model_endpoint check and stage11_preflight's credentials check.
    ...
    def _relay_matches(self, provider: str, model: str, upstream: str,
        ...
        return (
            labels.get("com.tempera.owner") == "benchmark-model-relay"
            and labels.get("com.tempera.provider") == provider
            and labels.get("com.tempera.model") == model
            and labels.get("com.tempera.upstream_sha256") == upstream_hash
            and labels.get("com.tempera.allowed_endpoints_sha256") == allowed_hash
            and values.get("TEMPERA_MODEL_UPSTREAM") == upstream
            and values.get("TEMPERA_PROVIDER") == provider
            and values.get("TEMPERA_MODEL") == model
            and values.get("TEMPERA_MODEL_ALLOWED_ENDPOINTS") == ",".join(sorted(allowed))
        )

    def _relay_owned(self) -> bool:
        _, labels = self._relay_inspect()
        return labels.get("com.tempera.owner") == "benchmark-model-relay"
    ...
    def _ensure_model_relay(self) -> None:
        ...
        if exists and not self._relay_matches(provider, model, upstream, allowed):
            if not self._relay_owned():
                raise TopologyError("model relay configuration mismatch: existing relay is not benchmark-owned")
            self._docker("rm", "-f", self.model_relay)
            exists = False
        if not exists:
            self._docker(
                "run", "-d", "--name", self.model_relay, "--network", CONTROL_NETWORK,
                "--label", "com.tempera.owner=benchmark-model-relay",
                "--label", f"com.tempera.provider={provider}",
                "--label", f"com.tempera.model={model}",
                "--label", f"com.tempera.upstream_sha256={upstream_hash}",
                "--label", f"com.tempera.allowed_endpoints_sha256={allowed_hash}",
                "-e", f"TEMPERA_MODEL_UPSTREAM={upstream}",
                "-e", f"TEMPERA_PROVIDER={provider}", "-e", f"TEMPERA_MODEL={model}",
                "-e", f"TEMPERA_MODEL_ALLOWED_ENDPOINTS={','.join(sorted(allowed))}",
    ...
    def cleanup(self) -> None:
        """Remove only a gateway container created with benchmark ownership labels."""
        name = self.topology.gateway
    ...
    def smoke(self, runtime: ContainerAgentRuntime) -> dict[str, object]:
        """GET the gateway's dedicated health route (never recorded as an
        agent event -- see gateway.py's gateway_health()) using the real
        container runtime path."""
        self.validate()
        result = runtime.exec([
            "curl", "-sS", "-o", "/dev/null", "-w", "%{http_code}",
            f"http://{self.topology.gateway}:8080/__tempera_gateway_health__",
        ])
        if result.stdout.strip() != "200":
            raise TopologyError(f"gateway smoke returned {result.stdout.strip()!r}")
        return {"command": "GET /__tempera_gateway_health__", "status": result.stdout.strip(), "stderr": result.stderr}

    def isolation_report(self, runtime: ContainerAgentRuntime, *, timeout: float = 10) -> dict[str, str]:
        """Prove from inside the attacker container what it can and cannot reach.

        This is a live, from-the-attacker's-own-network-namespace check (not
        just a static Docker network membership inspection like
        :meth:`validate`): it actually attempts a direct connection to the
        target and the model relay from the attacker container itself, so a
        misconfiguration that ``validate()`` cannot see (for example the
        target being reachable via a route that isn't a Docker network
        attachment) would still be caught here.
        """
        ...
        gateway_access = probe(f"http://{self.topology.gateway}:8080/__tempera_gateway_health__")
