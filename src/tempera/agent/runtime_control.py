"""Small process runtimes used by the runner."""
...
DEFAULT_ATTACKER_CONTAINER = "tempera-attacker"
CONTAINER_ENV_ALLOWLIST = {
    "TEMPERA_RUN_ID", "TEMPERA_SCENARIO_ID", "TEMPERA_PROVIDER", "TEMPERA_MODEL",
    "TEMPERA_MODEL_ENDPOINT",
}
...
class ContainerAgentRuntime:
    ...
    def run_tool(self, command: Sequence[str], *, timeout: float = 60,
        ...
        result = self.exec(["python3", "-m", "tempera.agent.tool_runner", "--timeout", str(timeout), *command], timeout=timeout + 5)
        try:
    ...
    def run(self, episode_func: Callable, mission: str, gateway_url: str,
        ...
        """Run one agent episode inside this attacker container.

        Unlike ``HostAgentRuntime.run``, ``episode_func`` (``run_episode``) is
        never called in this process: calling it here would execute the
        agent loop -- including every LLM call and gateway HTTP request --
        on the Host, which violates the architecture ("Host never runs the
        real agent process"). Instead this writes the episode's full input
        as JSON into the shared run_dir, invokes
        ``python3 -m tempera.agent.container_episode`` inside the attacker
        container via ``docker exec``, and reads back whatever that process
        wrote to ``container_outcome.json``.

        ``episode_func`` is still accepted (and intentionally unused beyond
        an identity assertion) to keep this call-site interchangeable with
        ``HostAgentRuntime.run`` from the runner's point of view.
        """
        if run_dir is None:
        ...
        # model_endpoint must come from the caller's explicit kwarg only,
        # never a Host-process os.environ fallback: the real runner
        # (src/tempera/runner.py's container branch) always passes it
        # explicitly, and TEMPERA_MODEL_ENDPOINT in the HOST's own shell is
        # meaningless orchestration noise (it only has real meaning inside
        # the attacker container, where container_episode.py sets it after
        # reading container_input.json). Falling back to it here would
        # silently mask a real bug where the runner failed to pass
        # model_endpoint -- exactly the "container executes without a
        # verified relay endpoint" failure mode this check exists to
        # prevent -- if an operator's shell happens to have a stale/
        # leftover TEMPERA_MODEL_ENDPOINT set from an unrelated session.
        ...
        try:
            result = self.exec(
                ["python3", "-B", "-m", "tempera.agent.container_episode",
