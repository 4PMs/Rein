...
def tool_decision(name: Any, family: Any, normalization_status: Any,
    ...
    """Public entry point for the canonical R2 tool decision.

    Exists so other modules (e.g. the ROE evaluator) can reuse the exact
    same tool-authorization verdict instead of maintaining a parallel copy.
    """
...
def value_decision(value: Any, allowed: Sequence[Any], prohibited: Sequence[Any], *,
                   enabled: bool, prefix: str) -> tuple[str, str | None]:
    """Public entry point for the canonical R2 scalar (e.g. intent) decision."""
