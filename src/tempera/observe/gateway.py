...
def create_app(upstream: str, run_id: str, actor: str, event_sink: EventSink,
    ...
    @app.route("/__tempera_gateway_health__")
    def gateway_health() -> Response:
        # A pure liveness check the Host/topology smoke tests use to verify
        # the gateway container itself is up and to probe attacker->gateway
        # reachability from inside the attacker container (see
        # ForcedGatewayTopology.smoke()/isolation_report()). It deliberately
        # never touches the upstream target and is NEVER recorded as an
        # events.jsonl entry: unlike forward_request() below, it returns
        # before any Observer.normalize()/event_sink() call. Without this
        # separate route, every smoke/isolation probe would have to go
        # through forward_request() against the real upstream, which
        # silently polluted the agent's own events.jsonl with orchestration
        # traffic the agent never sent -- verified live: this was
        # discovered because it inflated metrics.steps and produced
        # spurious R1 (not_allowed target) and R3 (discovery activity)
        # violations attributed to the agent for a plain "GET /" the
        # runner's own pre-flight smoke check made, not the agent.
    ...
    def forward_request(path: str) -> Response:
        ...
        # Flask/Werkzeug percent-decodes the incoming path (including
        # characters like '#' and non-ASCII bytes) before this handler runs.
        # Passing that decoded string straight to `requests` is unsafe: a
        # literal '#' is parsed as the start of a URL fragment by
        # requests/urllib3's URL handling, silently truncating everything
        # after it before the request ever reaches the upstream target
        # (verified live: a request for a path containing a decoded '#'
        # arrived at Juice Shop shortened at the '#', changing which
        # resource was actually requested). Re-encoding the path (but not
        # the already-decoded query string, which is forwarded as-is)
        # before building the outbound URL restores the original
        # percent-encoded octets so the upstream server sees exactly what
        # the client asked for. `safe="/"` preserves path segment
        # boundaries; ordinary ASCII paths are unaffected (quote() is a
        # no-op for them).
        ...
        invocation_token = next(
            (value for key, value in request.headers.items()
             if key.lower() == "x-tempera-invocation-token"), None,
        )
...
def main() -> None:
    ...
    observer = _load_observer(args.observer, markers=markers) if args.observer else None
    app = create_app(
