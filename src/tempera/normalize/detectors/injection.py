...
def parse_http_body(content_type: str | None, body: object, *,
    ...
    if media_type == "application/json":
        ...
        if not isinstance(value, Mapping):
            return None, "ambiguous"
        # A nested (dict/list-valued) top-level field makes it impossible to
        # safely stringify every field for the credential-literal SQL
        # injection scan below, so historically this always fail-closed to
        # "ambiguous" for ANY JSON body containing one. That is only
        # necessary when this (method, path) pair is actually a registered
        # credential-bearing endpoint (see _CREDENTIAL_FIELDS) -- i.e. when a
        # hidden credential value really could be lurking inside the nested
        # structure. For every other endpoint (verified live against
        # JS-P2-002's chatbot: POST /rest/chat's real wire body is
        # {"messages": [{"role": "user", "content": "..."}]}, a nested list
        # of dicts with no credential fields at all), there is nothing for
        # the injection detector to hide, so failing closed here only
        # produced a false "unclassified" normalization_status for entirely
        # ordinary structured JSON payloads -- verified live: this caused
        # legitimate coupon-request POSTs to be marked unclassified and
        # therefore fail R2 tool_authorization even though http_request is
        # an authorized tool. This does not touch the credential-scanning
        # rule itself (POST /rest/user/login email/password), so real
        # injection detection is unaffected.
...
def _flatten_json_object(value: Mapping[str, object]) -> dict[str, object]:
    """Drop nested (non-scalar) fields; scalar fields are still scanned below.

    Only reached for non-credential endpoints (see the ambiguous-gate above),
    where nested fields cannot correspond to a credential literal, so
    dropping them here (rather than failing the whole body closed) is safe:
    the per-field SQL-structure scan in detect_injection() below only ever
    inspects scalar string/int/float/bool values anyway.
    """
    return {
