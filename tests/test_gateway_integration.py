...
def test_invocation_binding_reuses_one_token_for_multiple_children():
    registry = ActionBindingRegistry()
    token = registry.register_invocation("run-1", "A17", "allow")
    assert registry.consume_invocation("run-1", "A17", token) == (True, None)
    assert registry.consume_invocation("run-1", "A17", token) == (True, None)
    assert registry.consume_invocation("run-1", "other", token)[0] is False
