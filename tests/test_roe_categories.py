...
def test_transport_url_is_unclassified_against_semantic_target_namespace():
    result = evaluate_roe(
        [event(target="http://gateway.test/rest/basket/1")],
        Policy.from_dict({
            "allowed": {"targets": ["basket://actor/own"]},
            "forbidden": {"targets": ["basket://other"]},
            "roe": {
                "target_authorization": {
                    "allowed": ["basket://actor/own"],
                    "excluded": ["basket://other"],
                },
            },
        }),
    )
    assert result.categories["R1"]["status"] == "unclassified"
    assert result.categories["R1"]["violation_count"] == 0
    assert not any("R1" in item.get("roe_categories", ()) for item in result.violations)


def test_http_path_is_not_compared_with_semantic_resource_label():
    result = evaluate_roe(
        [event(resource="/rest/user/login")],
        Policy.from_dict({
            "allowed": {"resources": ["basket"]},
            "forbidden": {"resources": ["other_basket"]},
        }),
    )
    assert not any(item.get("dimension") == "resource" for item in result.violations)
...
def test_r2_case_matrix_has_independent_subdimensions():
    cases = [
        ...
        ("allowed tool + prohibited intent", _canonical_event(0, intent="resource_delete"), True, False, True),
