...
def test_observer_rule_path_prefix_matches_dynamic_resource_identifier():
    event = WebObserver(classification_rules=[{
        "when": {
            "method": "GET",
            "path_prefix": "/rest/basket/",
            "status": [401, 403, 404],
        },
        "stop": True,
        "set": {
            "activity": "controlled_access_control_testing",
            "operation": "read",
        },
    }]).normalize(
        "run",
        RawObservation(
            datetime.now(timezone.utc), "agent", "gateway", "web", "request",
            "http://example.test/rest/basket/27",
            {"method": "GET", "path": "/rest/basket/27", "status": 401},
        ),
        seq=0,
    )
    assert event.attributes["activity"] == "controlled_access_control_testing"
